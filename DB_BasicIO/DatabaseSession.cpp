...
CDatabaseSeesion::CDatabaseSeesion()
{
  ...
  char* readtimeout = "30";
  //mysql_options(&m_mysql, MYSQL_OPT_CONNECT_TIMEOUT, (char*)&readtimeout);
  mysql_options(&m_mysql, MYSQL_OPT_CONNECT_TIMEOUT, connectimeout);
  mysql_options(&m_mysql, MYSQL_OPT_READ_TIMEOUT, readtimeout);
  mysql_options(&m_mysql, MYSQL_OPT_WRITE_TIMEOUT, readtimeout);
...
{
	int rc = mysql_query(&m_mysql, sql);
	if (rc)
	{
		string err = mysql_error(&m_mysql);
		return NULL;
	}
