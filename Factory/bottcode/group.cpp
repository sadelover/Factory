...
void CKGroup::RemoveItems (CObArray &cItemList, DWORD dwCount, bool bDelete /* = true */)
{
	...
	if (m_pServer->IsConnected ())
	{
		...
		ZeroMemory (&m_dwBuffer, sizeof (m_dwBuffer));
	...
	if (m_pServer->IsConnected () && m_pIItemMgt)
	{
		...
		try
		{
			...
			hr = m_pIItemMgt->SetActiveState (
				cdwItems,	// Item count
				m_dwBuffer,	// Server handles for items
...
bool CKGroup::ReadSync (CObArray &cItemList, DWORD cdwItems, bool bDeviceRead, bool bPostMsg /* = true */)
{
	...
	if(m_pISync==NULL)
	{
		OutOPCReadErrLog(_T("ERROR:m_pISync is Null."));
		return false;
	}

	if(!m_pServer->IsConnected ())
	{
		OutOPCReadErrLog(_T("ERROR:m_pServer connect fail."));
		return false;
	}
		...
		try
		{
			...
			{
				// Check results:
				if(pErrors==NULL || pValues==NULL)//golding add, 如果opc服务中途中断，会返回在pErrors=NULL, pValues = NULL.
				{
					bReadSuccess = false;
				}
				else
				{
					...
					for (dwIndex = 0; dwIndex < cdwItems; dwIndex++)
					{
						...
						if(pErrors && SUCCEEDED(pErrors[dwIndex]))
						{
							...
							// Update item data:
							pItem->UpdateData (pValues [dwValueIndex].vDataValue, 
								pValues [dwValueIndex].wQuality, pValues [dwValueIndex].ftTimeStamp);
						}
						else
						{
							bReadSuccess = false;//golding test: 当opc点没有配置在下位机中时，会到这里
							//_tprintf(L"ERROR:OPCServer Can't find %s\r\n",pItem->GetItemID());
							CString strErr;
							strErr.Format(_T("ERROR:OPCServer Can't find %s."),pItem->GetItemID());
							OutOPCReadErrLog(strErr);
							continue;
						}
						// Should clear variant before we free it later:
						VariantClear (&pValues [dwValueIndex].vDataValue);
						dwValueIndex++;
					}
				}
			}
			...
			{
				if(hr == 0x80010108)  //被调用的对象已与其客户端断开连接。
				{
					//_tprintf(L"ERROR:Called object(opc) has been disconnected from its clients.\r\n");
					OutOPCReadErrLog(_T("ERROR:Called object(opc) has been disconnected from its clients."));
				}
				else if(hr == 0x800706be)		//远程过程调用失败。
				{
					OutOPCReadErrLog(_T("ERROR:Call the remote procedure(opc) fails."));
				}
				else if(hr == 0x800706ba)		//RPC 服务器不可用
				{
					OutOPCReadErrLog(_T("ERROR:RPC server(opc) is unavailable."));
				}
			}
		}
		catch (...)
		{
			...
			OutOPCReadErrLog(_T("ERROR:ReadSync exception handler invoked."));
			if (m_pServer->IsConnected ())
			{
				// We not disconnected so why was an exception thrown?
				ASSERT (FALSE);
...
void CKGroup::WriteSync (CObArray &cItemList, CStringArray &cValues, DWORD cdwItems)
{
	...
	if (m_pServer->IsConnected () && m_pISync)
	{
		...
		// Return if allocation failed:
		if (m_dwBuffer == NULL || pValues == NULL)
		{
			ASSERT (FALSE);
			return;
		}
		...
		for (dwIndex = 0, cdwSentItems = 0; dwIndex < cdwItems; dwIndex++)
		{
			...
			if (pItem->GetDataType () & VT_ARRAY)
			{
				...
				if (MapStringValToArrayVariant (cValues [cdwSentItems], pItem->GetValue (), &pValues [cdwSentItems]))
					m_dwBuffer [cdwSentItems++] = pItem->GetServerHandle ();
				else
				{
	...
	if ( m_pServer->IsConnected () && m_pIAsync )
	{
		...
		try
		{
			...
			hr = m_pIAsync->Read (
				...
				m_dwBuffer,		// Item server handles	
	...
	if (m_pServer->IsConnected () && m_pIAsync)
	{
		...
		try
		{
			...
			hr = m_pIAsync->Write (
				...
				m_dwBuffer,			// Server item handles
...
static double wtof (LPCWSTR lpszValue)
{
	...
	size_t   i;
	wcstombs_s(&i, szBuffer, 256, lpszValue, sizeof (szBuffer) / sizeof (char));
...
bool CKGroup::OutOPCReadErrLog( CString strOut )
{
	...
	CString strLogPath;
	strLogPath.Format(_T("%s\\OPCReadErr.log"),strFilePath);
	CStdioFile	ff;
