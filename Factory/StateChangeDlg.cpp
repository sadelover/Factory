...
IMPLEMENT_DYNAMIC(CStateChangeDlg, CSkinDlg)

  
CStateChangeDlg::CStateChangeDlg(CCanvasDlg* pCanvalDlg, CWnd* pParent /*=NULL*/)
	: CSkinDlg(CStateChangeDlg::IDD, pParent)
	, m_nValue(0)
	, m_nPicID(0)
	, m_nFrame(0)
	, m_nInterval(0)
...
CStateChangeDlg::~CStateChangeDlg()
{
	if(m_pAnimationView)
	{
...
void CStateChangeDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CStateChangeDlg, CSkinDlg)
	ON_WM_PAINT()
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_BUTTON_PIC, &CStateChangeDlg::OnBnClickedButtonPic)
	ON_BN_CLICKED(IDC_BUTTON_ANI, &CStateChangeDlg::OnBnClickedButtonAni)
	ON_BN_CLICKED(IDOK, &CStateChangeDlg::OnBnClickedOk)
	ON_WM_ERASEBKGND()
END_MESSAGE_MAP()


// CStateChangeDlg 消息处理程序


BOOL CStateChangeDlg::OnInitDialog()
{
...
void CStateChangeDlg::OnBnClickedButtonPic()
{
...
void CStateChangeDlg::OnBnClickedButtonAni()
{
	CMetaFileSelectDlg dlg(this);
...
bool CStateChangeDlg::CompareFunction( const int pElementA,const int pElementB )
{
	return pElementA < pElementB;
}

void CStateChangeDlg::OnBnClickedOk()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData(TRUE);
...
void CStateChangeDlg::OnTimer( UINT_PTR nIDEvent )
{
...
void CStateChangeDlg::SetProjectSqlPointer( CProjectSqlite* pProjectSqlite )
{
	m_pProjectSqlite = pProjectSqlite;
}

void CStateChangeDlg::ChangePictureByID()
{
...
BOOL CStateChangeDlg::OnEraseBkgnd(CDC* pDC)
{
