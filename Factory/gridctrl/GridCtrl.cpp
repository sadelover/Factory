...
std::string CGridCtrl::WideCharToAnsi(const wchar_t* source)
{
...
std::wstring CGridCtrl::UTF8ToWideChar(const char* source)
{
	if(source==NULL)
		return L"";
	//预转换，得到所需空间的大小 
	int wcsLen = ::MultiByteToWideChar(CP_ACP, NULL, source, (int)strlen(source), NULL, 0); 
	//分配空间要给'\0'留个空间，MultiByteToWideChar不会给'\0'空间 
	wchar_t* wszString = new wchar_t[wcsLen + 1]; 
	//转换 
	::MultiByteToWideChar(CP_ACP, NULL, source, (int)strlen(source), wszString, wcsLen); 
	//最后加上'\0' 
	wszString[wcsLen] = L'\0'; 

	std::wstring result = wszString;
	delete[] wszString;

	return result;
}
#include <locale>
std::wstring StringToWstring(const std::string s)// string转wstring
{ 
								...
								BOOL bSelectPastedCells /*=TRUE*/)
{
	...
	int nnsize = ::GlobalSize(hmem);
	char * szBufferNew = new char[nnsize];
	sf.Read(szBufferNew, (UINT)::GlobalSize(hmem));

	//std::string sszBufferNew = szBufferNew;
	//std::wstring wstrtemp = StringToWstring(sszBufferNew);

	std::wstring wstrTemp = UTF8ToWideChar(szBufferNew);
    ...
    CString strText = wstrTemp.c_str();
    delete szBuffer;
...
//    ASSERT(nRow >= 0 && nRow < m_nRows);
	...
	if(nRow>=m_arRowHeights.GetSize())
		return -1;
...
int CGridCtrl::GetColumnWidth(int nCol) const
{
	...
	int xx = m_arColOrder[nCol];
	if(xx>=m_arColWidths.GetSize())
		return -1;
