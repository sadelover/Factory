...
static int g_nReadMode = 0;			//默认0是读index  1是读ip
...
uint32_t conver_add_to_index( BACNET_ADDRESS * src )
{
	if(src)
	{
		char sadr_string[20]={0};
		sprintf(sadr_string, "%d.%d.%d.%d", (int)src->mac[0],(int)src->mac[1], (int)src->mac[2], (int)src->mac[3]);
		return inet_addr(sadr_string);
	}
	return 1;
}
    ...
    BACNET_ADDRESS * src)
{
	...
	//如果是读IP的话 要将add地址前四位转成index
	if(g_nReadMode == 1)	
		device_id = conver_add_to_index(src);
